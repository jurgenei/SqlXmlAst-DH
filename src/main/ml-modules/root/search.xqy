...
declare function local:results($search, $start, $amount) {
    <dl> {
        ...
        return
            <div>
                <dt><a href="/search.xqy?search={$search}&amp;amount={$amount}&amp;start={xs:int($start)}&amp;docurl={$docuri}">{$docuri}</a></dt>
                <dd>{for $e in $content
                let $v := string($e)
                return
                    if ( local-name($e) = 'highlight') then <b>{$v}</b>
                    else $v
                }</dd>
            </div>
    }</dl>
    ...
    <body>
        <div>
            <form action="/search.xqy" method="get">
                ...
                then (
                    ...
                    (
                        <a href="/search.xqy?search={$search}&amp;amount={$amount}&amp;start={xs:int($start)}">Back to search results</a>
                    )
                }
            </form>
        </div>
        <div>
            {
                if ($docurl ne "")
                then (
                    xdmp:xslt-invoke("/render.xslt", doc($docurl)),
                    <hr/>,
                    <div><pre>{fn:unparsed-text($docurl)}</pre></div>
                )
                else (
        ...
        <div>
            ...
            }
            {
                ...
                then (
                    <a href="/search.xqy?search={$search}&amp;amount={$amount}&amp;start={xs:int($start) - xs:int($amount)}">previous {$amount}&nbsp;&nbsp;&nbsp;</a>
                )
                else (
                    ''
                )
...
then (
<a href="/search.xqy?search={$search}&amp;amount={$amount}&amp;start={xs:int($start) + xs:int($amount)}">next {$amount}</a>
)
else (
''
)
